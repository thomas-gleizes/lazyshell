...
type Emulator struct {
	...
	// Indicates if the terminal is closed. Read (from an arbitrary reader
	// goroutine) and Close (from whoever tears the emulator down) run
	// concurrently by design in lazyshell — Close is the only way to unblock
	// a goroutine parked in Read on e.pr — so this must be an atomic, not a
	// plain bool, or -race flags it every run. Patched locally in
	// third_party/charmbracelet/x/vt; see third_party/charmbracelet/x/vt/README.md.
	closed atomic.Bool
