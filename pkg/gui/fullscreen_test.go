...
func TestOutputPanelConsumes256ColourSequences(t *testing.T) {
	...
	view.SetContent(buildOutputFrame(sess, 0, false, "", -1, -1).content)
...
func TestOutputFrameCarriesTheCursorOnlyWhenTypedInto(t *testing.T) {
	...
	if buildOutputFrame(sess, 0, false, "", -1, -1).cursorShown {
		t.Error("cursor shown while not in pass-through")
	}

	if buildOutputFrame(sess, 5, true, "", -1, -1).cursorShown {
...
func TestOutputFrameHonoursHiddenCursor(t *testing.T) {
	...
	if buildOutputFrame(sess, 0, true, "", -1, -1).cursorShown {
...
func TestSessionsPanelGutterMarkers(t *testing.T) {
	...
	lines := strings.Split(strings.TrimRight(sessionsPanelContent([]*session.Session{quiet, ringing, editing}, testMarkers, "", nil, nil), "\n"), "\n")
	if len(lines) != 3 {
...
func TestSessionsPanelActivityMarker(t *testing.T) {
	...
	content := sessionsPanelContent([]*session.Session{quiet, busy, watched}, testMarkers, watched.ID, nil, nil)
...
func TestSessionsPanelExitResult(t *testing.T) {
	...
	got := sessionsPanelContent([]*session.Session{sess}, testMarkers, "", nil, nil)
...
func TestSessionsPanelShowsTheTerminalTitle(t *testing.T) {
	...
	if got := sessionsPanelContent([]*session.Session{sess}, testMarkers, "", nil, nil); !strings.Contains(got, sess.Cwd) {
...
func TestSessionsPanelSkipsUnchangedRedraws(t *testing.T) {
	...
	content := sessionsPanelContent(gui.sessions.List(), testMarkers, "", nil, nil)
...
func TestOutputFrameIsStableWhileNothingHappens(t *testing.T) {
	...
	second := buildOutputFrame(sess, 0, true, "", -1, -1)
	...
	if buildOutputFrame(sess, 0, true, "", -1, -1) == first {
