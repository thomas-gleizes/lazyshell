...
func TestLanguageEnglishTranslatesTheInterface(t *testing.T) {
	...
	if got := sessionsPanelContent(nil, testMarkers, "", gui.tr); !strings.Contains(got, "No sessions") {
...
func TestDefaultLanguageStaysFrench(t *testing.T) {
	...
	if got := sessionsPanelContent(nil, testMarkers, "", gui.tr); !strings.Contains(got, "Aucune session") {
