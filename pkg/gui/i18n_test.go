...
func TestLanguageEnglishTranslatesTheInterface(t *testing.T) {
	...
	if got := sessionsPanelContent(nil, testMarkers, "", nil, nil, gui.tr, 0); !strings.Contains(got, "No sessions") {
...
func TestDefaultLanguageStaysFrench(t *testing.T) {
	...
	if got := sessionsPanelContent(nil, testMarkers, "", nil, nil, gui.tr, 0); !strings.Contains(got, "Aucune session") {
